...
/**
 * Byte representation - Serialization/Deserialization.
 * Primitive types:
 * (), u64, u128 e f32
 * Container types:
...
pub trait ByteRpr {
    ///
    /// Allocates the object into a vector of u8
    ///
    ...
    ///
    /// Writes the vector representation using a writer
    ///
    fn as_byte_rpr(&self, buff: &mut dyn std::io::Write) -> usize;
    ///
    /// Recreates the object given an array of bytes
    ///
    fn from_byte_rpr(bytes: &[u8]) -> Self;
}

/**
 * Get the segment len for types that have fixed byte len.
 * E.g. (Vec<T> doesn't have fixed type, but u32 has.)
 */
    ...
    T: ByteRpr + FixedByteLen,
{
    ...
    fn from_byte_rpr(bytes: &[u8]) -> Self {
        match bytes[0] {
            ...
            0 => None,
            _ => unreachable!("Invalid byte pattern"),
        }
    }
}
    ...
    T: ByteRpr + FixedByteLen,
{
    ...
    fn from_byte_rpr(bytes: &[u8]) -> Self {
        ...
        while start < bytes.len() {
            deserialized.push(T::from_byte_rpr(&bytes[start..end]));
            start = end;
...
impl ByteRpr for () {
    fn as_byte_rpr(&self, _buff: &mut dyn std::io::Write) -> usize {
        0
    }
